...
<template>
  <div class="membership-voting-actions">
    <!-- Active voting: show vote buttons -->
    <div v-if="isActive" class="membership-voting-actions__vote">
      <Button 
        @click="handleVoteFor"
        :disabled="voteComposable.isPending.value || voteComposable.isConfirming.value || !address"
        class="membership-voting-actions__button membership-voting-actions__button--for"
      >
      ...
      <Button 
        ...
        :disabled="voteComposable.isPending.value || voteComposable.isConfirming.value || !address"
        class="membership-voting-actions__button membership-voting-actions__button--against"
      >
        {{ voteComposable.isPending.value || voteComposable.isConfirming.value ? t('voting.buttons.voting') : t('voting.buttons.voteAgainst') }}
      </Button>
    </div>

    <!-- Ended but not finalized: show execute button -->
    <div v-else-if="hasEnded" class="membership-voting-actions__execute">
      <Button 
        @click="handleExecute"
        :disabled="executeComposable.isPending.value || executeComposable.isConfirming.value || !address"
        class="membership-voting-actions__button membership-voting-actions__button--execute"
      >
    ...
    <!-- Finalized: show result -->
    <div v-else class="membership-voting-actions__finalized">
      <p class="membership-voting-actions__status">
        {{ t('voting.messages.votingFinalized') }}
        <span 
          :class="[
            'membership-voting-actions__result',
            `membership-voting-actions__result--${resultStatus}`
          ]"
        >
...
<style lang="scss">
@use "../../assets/scss/membershipVoting/actions.scss";
</style>
