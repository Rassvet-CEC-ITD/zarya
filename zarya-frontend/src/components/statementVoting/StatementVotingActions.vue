...
<template>
  <div class="statement-voting-actions">
    <Toast 
      v-if="showToast"
      :type="toastType"
    ...
    <div v-if="isActive" class="statement-voting-actions__voting-section">
      <div class="statement-voting-actions__organ-selector">
        <h4 class="statement-voting-actions__organ-title">{{ t('voting.labels.selectOrgan') }}</h4>
        
        <div class="statement-voting-actions__organ-form">
          <div class="statement-voting-actions__form-group">
            <label class="statement-voting-actions__label" for="organType">
              {{ t('voting.form.organType') }}
            </label>
            <select
              id="organType"
              v-model.number="organSelection.organType"
              class="statement-voting-actions__select"
            >
              <option 
          ...
          <div class="statement-voting-actions__form-row">
            <div class="statement-voting-actions__form-group">
              <label class="statement-voting-actions__label" for="region">
            ...
            <div 
              ...
              class="statement-voting-actions__form-group"
            >
              <label class="statement-voting-actions__label" for="organNumber">
              ...
              <input
                ...
                min="0"
                placeholder="0"
                class="statement-voting-actions__input"
              />
            </div>
          </div>

          <div class="statement-voting-actions__organ-info">
            <span class="statement-voting-actions__label">{{ t('voting.form.organIdentifier') }}</span>
            <strong class="statement-voting-actions__identifier">{{ identifier }}</strong>
          </div>
        </div>
      </div>

      <div class="statement-voting-actions__vote-buttons">
        <Button
          variant="success"
          :disabled="!address || isCastPending || isConfirmingCast"
          @click="handleVote(true)"
          class="statement-voting-actions__button"
        >
    ...
    <div v-if="canExecute" class="statement-voting-actions__execute">
      <Button
        variant="primary"
    ...
    <div v-if="voting.finalized" class="statement-voting-actions__finalized">
      <p class="statement-voting-actions__status">
        {{ t('voting.messages.votingFinalized') }} 
        <span :class="['statement-voting-actions__result', voting.success ? 'statement-voting-actions__result--success' : 'statement-voting-actions__result--failed']">
...
<style lang="scss">
@use "../../assets/scss/statementVoting/actions.scss";
</style>
